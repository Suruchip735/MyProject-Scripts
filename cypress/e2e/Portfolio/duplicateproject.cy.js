...
describe('Project Creation Test Suite', () => {
  ...
  const randomPair =
    projectNameID[Math.floor(Math.random() * projectNameID.length)];
  ...
  it('Create a project with a duplicate name within the same portfolio should show error', () => {
    ...
    cy.get(selectors.projectNameInput).type(randomPair.key);
    cy.get(selectors.projectIdInput).type(String(randomPair.value));
